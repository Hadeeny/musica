...
const NowPlaying = () => {
  ...
  useEffect(() => {
    // console.log(audioEl.currentTime.currentTime, audioEl.current.duration - 2);
    if (audioEl.current.currentTime == audioEl.current.duration - 2) {
  ...
  useEffect(() => {
    const songDuration = Math.floor(audioEl.current.duration);
    const seconds = Math.floor(audioEl.current.currentTime);
    if (songDuration - 2 <= seconds) {
      dispatch(goToNextSong());
    }
    if (repeatSong) {
      audioEl.current.loop = true;
    } else audioEl.current.loop = false;
  });

  // Link song time to progressbar
  const changeRange = () => {
  ...
  // Moves progress bar as song plays
      ...
      >
        ...
        <div className="h-14">
          ...
          <div className="flex justify-center gap-12">
            <BiShuffle className="cursor-pointer hidden text-gray-400 lg:flex text-2xl" />
            <GiPreviousButton
              onClick={() => {
            ...
            <TbRepeatOnce
              ...
              className={`hidden ${
                repeatSong && "text-gray-400"
              } text-2xl cursor-pointer lg:flex`}
            />
          </div>
